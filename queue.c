...
struct Linkedlist{
  //its complete linked list so here we want head, tail, its complete size of linked list(its for queue and queue)
...
int main(){
  ...
  queue_push(queue1, 12);
  queue_push(queue1, 13);
  queue_push(queue1, 15);
  int top_value = queue_top(queue1);
  print(top_value);
  queue_pop(queue1);
